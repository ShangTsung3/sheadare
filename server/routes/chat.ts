...
const CHAT_PROMPT = `შენ ხარ "შეადარე"-ს AI ასისტენტი — ქართული ფასების შედარების აპლიკაციის.
შენი ამოცანაა დაეხმარო მომხმარებელს პროდუქტების ფასების შედარებაში.
...
const BASKET_PROMPT = `შენ ხარ "შეადარე"-ს AI ასისტენტი — ქართული ფასების შედარების აპლიკაციის.
...
function isElectronicsQuery(msg: string): boolean {
  const keywords = ['ტელეფონ', 'iphone', 'samsung', 'galaxy', 'ლეპტოპ', 'laptop', 'კომპიუტერ',
    ...
    'playstation', 'xbox', 'nintendo', 'პრინტერ', 'xbox', 'ps5', 'სმარტფონ'];
  const lower = msg.toLowerCase();
